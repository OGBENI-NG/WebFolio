...
export default function HomeFooter({data, themeColor}) {
  ...
  const renderLinksIcon = iconEl.map(iconEl => (
    <div 
      key={iconEl.id} 
      className='text-mainBg hover:text-mainBg/60'
    >
      {iconEl.icon({size: 35})}
    </div>
  ))
  ...
  const renderHeroContact = heroContact.map(info => (
    <div key={info.id} className={`text-lg font-semibold text-[#D5D5D5] flex items-center gap-2`}>
  ...
  return (
    <footer className='pt-[50px] text-mainBg pb-[120px] bg-[#243136]'>
      <h1 
