          ...
          >
            ...
            <div className='flex items-center ml-auto gap-4'>
              <button 
                ...
                <FaChevronDown 
                  className={`transition-[1s] text-lg md:text-2xl lg:text-lg
                  ${openId === item.id ? '-rotate-360' : 'rotate-180'}`}/>
              </button>
            </div>
          </div>
          {/* Render description only if isShow is true */}
          <div className='absolute xl:w-[469px] z-[999]'>
            <div className={` ${layoutTheme} rounded-b-lg shadow-md 
              ${openId === item.id ? 'animate-fadeInDown' : 'animate-fadeInClose'} overflow-hidden`}
            >
              <div className='p-4 md:p-6 lg:pb-5'>
